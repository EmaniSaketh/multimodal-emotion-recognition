...
def train(args):
    ...
    ckpt_path    = "checkpoints/text_best.pt"
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for ids, mask, y in tqdm(train_loader, desc=f"Epoch {epoch}/{args.epochs}"):
        ...
        t_acc  = correct / total
        v_loss, v_acc, _, _ = evaluate(model, val_loader, criterion, device, "text")
    ...
    _, test_acc, preds, labels = evaluate(model, test_loader, criterion, device, "text")
    print(f"\n=== Test Accuracy (Text): {test_acc:.4f} ===\n")
    print_report(labels, preds)

    plot_curves(train_losses, val_losses, train_accs, val_accs,
                "Text Pipeline", "Results/plots/text_curves.png")
    plot_confusion(labels, preds, "Text Confusion Matrix",
                   "Results/plots/text_confusion.png")
    ...
    pd.DataFrame({"epoch": range(1, len(train_losses)+1),
                ...
                ).to_csv("Results/tables/text_history.csv", index=False)
    with open("Results/tables/text_test_acc.json", "w") as f:
